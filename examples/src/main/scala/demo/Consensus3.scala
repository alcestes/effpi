...
// Does not compile if the continuation of In is invariant :-(
...
def endPhase[Peers <: Seq[OutChannel[Msg[?,?]]]]
            (id: Int, ci: InChannel[Msg[?,?]], peers: Peers,
             n: Nat, nA: Nat, nB: Nat): EndPhase[id.type, ci.type, Peers, n.type, nA.type, nB.type] = {
  compare(nA, nB) match {
...
def phase[Peers <: Seq[OutChannel[Msg[?,?]]]]
         (id: Int, ci: InChannel[Msg[?,?]], peers: Peers, n: Nat, nA: Nat, nB: Nat)
         (implicit timeout: Duration): Phase[id.type, ci.type, Peers,
                                             n.type, nA.type, nB.type] = compare(size(peers), n) match {
  case _: Greater[?,?] => endPhase(id, ci, peers, n, nA, nB)
  case _: LessEqual[?,?] => receive(ci) { (msg: Msg[?,?]) => msg.payload match {
    case _: A => phase(id, ci, peers, succ(n): Succ[n.type], succ(nA): Succ[nA.type], nB)
    case _: B => phase(id, ci, peers, succ(n): Succ[n.type], nA, succ(nB): Succ[nB.type])
  } }
}
*/
